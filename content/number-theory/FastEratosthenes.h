 ...
 * Time: LIM=1e8 $\approx$ 0.6s
 * Time: LIM=1e9 $\approx$ 6.5s
 ...
 *
 * Replace sqrt(LIM) with the sqrt value to avoid erorr
 *
...
const int LIM = 1e6;
const int sqrt_LIM = 1e3;  // manually change this
bitset<LIM> isPrime;
#define pii pi
vi eratosthenes() {
  constexpr int S = sqrt_LIM;
  const int R = LIM / 2;
  vi pr = {2}, sieve(S + 1);
